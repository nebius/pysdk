"""Lightweight helpers for Python code generation.

This module provides small abstractions to build Python source files with
managed imports, indentation, and optional generated headers.
"""

import io
from collections.abc import Sequence
from keyword import iskeyword, issoftkeyword
from types import TracebackType
from typing import Any, Literal, TextIO

GENERATED_NOTE = """
Generated by nebius_generator.  DO NOT EDIT!
"""


class GenerationError(Exception):
    """Raised when code generation state is invalid."""

    pass


class ImportPath:
    """Represents an importable module path with an optional alias hint.

    :param import_path: Dotted module path.
    :param suggest_name: Optional alias to use for the import.
    """

    def __init__(
        self,
        import_path: str,
        suggest_name: str | None = None,
        *,
        localized: bool = False,
    ) -> None:
        self.import_path = import_path
        self.suggest_name = suggest_name
        self.localized = localized

    def __eq__(self, value: object) -> bool:
        """Return True if two import paths refer to the same module."""
        if isinstance(value, ImportPath) and value.import_path == self.import_path:
            return True
        if isinstance(value, str) and value == self.import_path:
            return True
        return False


class ImportedSymbol:
    """Name + import path pairing for a symbol.

    :param name: Symbol name.
    :param import_path: Module path or :class:`ImportPath`.
    """

    def __init__(self, name: str, import_path: ImportPath | str) -> None:
        self.name = name
        self.import_path = (
            import_path
            if isinstance(import_path, ImportPath)
            else ImportPath(import_path)
        )

    def __eq__(self, value: object) -> bool:
        """Return True if two symbols match by name and import path."""
        if (
            isinstance(value, ImportedSymbol)
            and value.import_path == self.import_path
            and value.name == self.name
        ):
            return True
        return False

    def as_type_hint(self) -> "TypeHint":
        """Return a :class:`TypeHint` wrapper for this symbol."""
        return TypeHint(self.name, self.import_path)


class TypeHint(ImportedSymbol):
    """Imported symbol that should be emitted as a type hint."""

    def as_symbol(self) -> ImportedSymbol:
        """Return a regular :class:`ImportedSymbol` for runtime emission."""
        return ImportedSymbol(self.name, self.import_path)


Token = str | TypeHint | ImportedSymbol | ImportPath | Any


class PyGenFile:
    """Builder for a Python source file with managed imports.

    :param import_path: Module path for the generated file.
    :param indent_sequence: Indentation string (defaults to 4 spaces).
    :param shebang_command: Optional shebang command (e.g., ``/usr/bin/env python``).
    :param generated_note: Optional header text to emit as comments.
    :param used_names: Names that should be considered reserved.
    :param docstring: Optional module docstring to emit.
    """

    def __init__(
        self,
        import_path: str | ImportPath,
        indent_sequence: str | None = None,
        shebang_command: str | None = None,
        generated_note: str | None = None,
        used_names: Sequence[str] | None = None,
        docstring: str | None = None,
        module: bool = False,
    ) -> None:
        """Create a Python file generator."""
        self._shebang_command = shebang_command
        self._generated_note = (
            generated_note if generated_note is not None else GENERATED_NOTE
        )
        self._import_path = (
            import_path if isinstance(import_path, str) else import_path.import_path
        )
        self._package_path = (
            self._import_path.rsplit(".", 1)[0] if module else self._import_path
        )
        self._used_names = set(used_names) if used_names is not None else set[str]()
        self._main_code = io.StringIO("")
        self._imports = dict[str, ImportPath]()
        self._imports_reversed = dict[str, str]()
        self._indent_sequence = (
            indent_sequence if indent_sequence is not None else "    "
        )
        self._indent = 0
        self._finalized = False
        self._docstring = docstring
        self.registry: ImportPath | None = None

    def is_local(self, import_path: ImportPath | str) -> bool:
        """Return True if the import path refers to this module."""
        import_path_str = (
            import_path.import_path
            if isinstance(import_path, ImportPath)
            else import_path
        )
        return import_path_str == self._import_path

    @property
    def import_path(self) -> str:
        """Return the module path represented by this generated file."""
        return self._import_path

    def append_used_names(self, used_names: Sequence[str]) -> None:
        """Record additional reserved names."""
        self._used_names = self._used_names.union(used_names)

    def suggest_name(
        self,
        suggestion: str,
        additional_names: Sequence[str] | None = None,
    ) -> str:
        """Return a non-conflicting import alias suggestion.

        :param suggestion: Preferred alias.
        :param additional_names: Additional names to avoid.
        :returns: Safe alias name.
        """
        ans = set(additional_names) if additional_names is not None else set[str]()

        def is_reserved(s: str) -> bool:
            return (
                s in self._imports_reversed
                or s in self._used_names
                or s in ans
                or iskeyword(s)
                or issoftkeyword(s)
            )

        if is_reserved(suggestion):
            inc = 1
            while True:
                new_suggest_name = f"{suggestion}_{inc}"
                if not is_reserved(new_suggest_name):
                    break
                inc += 1
            return new_suggest_name
        else:
            return suggestion

    def add_import(
        self,
        import_path: ImportPath | str,
        suggest_name: str | None = None,
    ) -> ImportPath:
        """Register an import and return the resolved :class:`ImportPath`.

        :param import_path: Module path or :class:`ImportPath`.
        :param suggest_name: Optional alias to use.
        :returns: Resolved :class:`ImportPath`.
        :raises GenerationError: If the file is already finalized.
        """
        if self._finalized:
            raise GenerationError("can't add imports to finalized file")
        import_path_str = (
            import_path.import_path
            if isinstance(import_path, ImportPath)
            else import_path
        )
        if self.is_local(import_path):
            return ImportPath(import_path_str)
        if import_path_str in self._imports:
            return self._imports[import_path_str]
        if suggest_name is None and isinstance(import_path, ImportPath):
            suggest_name = import_path.suggest_name
        if suggest_name is None:
            suggest_name = import_path_str.split(".")[-1]
        if not isinstance(import_path, ImportPath):
            import_path = ImportPath(import_path, suggest_name)

        suggest_name = self.suggest_name(suggest_name)

        import_path = ImportPath(
            import_path.import_path,
            suggest_name,
            localized=import_path.localized,
        )
        self._imports[import_path.import_path] = import_path
        self._imports_reversed[suggest_name] = import_path.import_path
        return import_path

    def set_docstring(self, docstring: str) -> None:
        """Set the module docstring."""
        self._docstring = docstring

    def indent(self) -> None:
        """Increase indentation for subsequent writes."""
        if self._finalized:
            raise GenerationError("can't indent finalized file")
        self._indent += 1

    def unindent(self) -> None:
        """Decrease indentation level."""
        if self._indent <= 0:
            raise GenerationError("can't unindent more")
        self._indent -= 1

    def __enter__(self) -> "PyGenFile":
        """Context manager that increases indentation."""
        self.indent()
        return self

    def __exit__(
        self,
        type_: type[BaseException] | None,
        value: BaseException | None,
        traceback: TracebackType | None,
    ) -> Literal[False]:
        """Context manager exit that decreases indentation."""
        self.unindent()
        return False

    def p(
        self,
        *args: Token,
        noindent: bool = False,
        add_eol: bool = True,
    ) -> None:
        """Write a line of code, resolving imports for known tokens.

        :param args: Tokens to write.
        :param noindent: Disable indentation for this line.
        :param add_eol: Append a newline.
        :raises GenerationError: If the file is already finalized.
        """
        if self._finalized:
            raise GenerationError("can't add lines to finalized file")
        if not noindent:
            self._main_code.write("".join([self._indent_sequence] * self._indent))
        for arg in args:
            if isinstance(arg, TypeHint):
                if self.is_local(arg.import_path):
                    self._main_code.write('"')
                    self._main_code.write(arg.name)
                    self._main_code.write('"')
                else:
                    import_path = self.add_import(arg.import_path)
                    self._main_code.write(str(import_path.suggest_name))
                    self._main_code.write(".")
                    self._main_code.write(arg.name)
            elif isinstance(arg, ImportedSymbol):
                if self.is_local(arg.import_path):
                    self._main_code.write(arg.name)
                else:
                    import_path = self.add_import(arg.import_path)
                    self._main_code.write(str(import_path.suggest_name))
                    self._main_code.write(".")
                    self._main_code.write(arg.name)
            elif isinstance(arg, ImportPath):
                if self.is_local(arg):
                    raise GenerationError(
                        f"can't add import path {arg.import_path} as an import to"
                        " itself"
                    )
                import_path = self.add_import(arg.import_path)
                self._main_code.write(str(import_path.suggest_name))
            else:
                self._main_code.write(f"{arg}")
        if add_eol:
            self._main_code.write("\n")

    def dump(self, writer: TextIO) -> None:
        """Write the generated file to a stream.

        :param writer: Output stream.
        """
        self._finalized = True
        if self._shebang_command is not None:
            writer.write("#!")
            writer.write(self._shebang_command)
            writer.write("\n")
        gen_note = self._generated_note.split("\n")
        for line in gen_note:
            writer.write("#")
            if line:
                writer.write(" ")
                writer.write(line)
            writer.write("\n")
        writer.write("\n")
        if self._docstring is not None:
            writer.write('"""')
            writer.write(self._docstring)
            writer.write('"""\n\n')
        sorted_imports = sorted(
            self._imports.values(),
            key=lambda x: x.import_path,
        )
        for _imp in sorted_imports:
            if _imp.localized:
                current = self._package_path.split(".")
                target = _imp.import_path.split(".")
                common = 0
                for current_part, target_part in zip(current, target):
                    if current_part != target_part:
                        break
                    common += 1
                relative = "." * (len(current) - common + 1)
                remainder = target[common:]
                if len(remainder) == 1:
                    writer.write("from ")
                    writer.write(relative)
                    writer.write(" import ")
                    writer.write(remainder[0])
                else:
                    writer.write("from ")
                    writer.write(relative + ".".join(remainder[:-1]))
                    writer.write(" import ")
                    writer.write(remainder[-1])
                writer.write(" as ")
                writer.write(str(_imp.suggest_name))
                writer.write("\n")
            else:
                writer.write("import ")
                writer.write(_imp.import_path)
                writer.write(" as ")
                writer.write(str(_imp.suggest_name))
                writer.write("\n")
        writer.write("#@ local imports here @#")
        writer.write("\n")
        writer.write("\n")
        writer.write(self._main_code.getvalue())
        writer.flush()

    def dumps(self) -> str:
        """Return the generated file as a string."""
        ret = io.StringIO("")
        self.dump(ret)
        lines = [line if line.strip() else "" for line in ret.getvalue().splitlines()]
        while lines and not lines[-1].strip():
            lines.pop()
        return "\n".join(lines) + "\n"
