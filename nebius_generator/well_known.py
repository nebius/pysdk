...
def configure_runtime_package(package: str) -> None:
    """Relocate runtime imports used by well-known type converters."""
    ...
    for converter in converter_list:
        for attribute in (
            ...
            symbol = getattr(converter, attribute)
            path = symbol.import_path.import_path
            if path == "nebius" or path.startswith("nebius."):
                symbol.import_path = ImportPath(package + path.removeprefix("nebius"))
